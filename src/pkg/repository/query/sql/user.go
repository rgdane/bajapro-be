...
func NewUserRepository() adapter.UserRepository {
...
func (repo *userRepository) WithTx(tx *gorm.DB) adapter.UserRepository {
	clone := repo.clone()
	clone.db = tx
	return clone
}

func (repo *userRepository) WithPreloads(preloads ...string) adapter.UserRepository {
	clone := repo.clone()
	clone.preloads = append(clone.preloads, preloads...)
	return clone
}

func (repo *userRepository) WithAssociations(associations ...string) adapter.UserRepository {
	clone := repo.clone()
	clone.associations = append(clone.associations, associations...)
	return clone
}

func (repo *userRepository) WithReplacements(replacements map[string]interface{}) adapter.UserRepository {
	clone := repo.clone()
	clone.replacements = replacements
	return clone
}

func (repo *userRepository) WithJoins(joins ...string) adapter.UserRepository {
	clone := repo.clone()
	clone.joins = append(clone.joins, joins...)
	return clone
}

func (repo *userRepository) WithWhere(query interface{}, args ...interface{}) adapter.UserRepository {
	clone := repo.clone()
...
func (repo *userRepository) WithOrder(order string) adapter.UserRepository {
	clone := repo.clone()
	clone.order = order
	return clone
}

func (repo *userRepository) WithLimit(limit int) adapter.UserRepository {
	clone := repo.clone()
	clone.limit = &limit
	return clone
}

func (repo *userRepository) WithCursor(cursor int) adapter.UserRepository {
	clone := repo.clone()
	clone.cursor = &cursor
	return clone
}

func (repo *userRepository) WithUnscoped() adapter.UserRepository {
	clone := repo.clone()
	clone.unscoped = true
	return clone
}
